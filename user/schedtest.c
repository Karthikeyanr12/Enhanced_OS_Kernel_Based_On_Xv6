...
print_stats_table(int count)
{
  ...
  for (int i = 0; i < count; i++) {
    printf("PID=%d PPID=%d STATE=%s PRIO=Q%d SIZE=%ld TICKS=%ld SCHED=%d NAME=%s\n",
...
main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < count; i++) {
    if (procs[i].pid == c1) {
      found_c1 = 1;
      if (procs[i].ppid != parent_pid) ppid_ok = 0;
      if (procs[i].cpu_ticks > 0) ticks_ok = 1;
      if (procs[i].num_sched > 0) sched_ok = 1;
    }
    if (procs[i].pid == c2) {
      found_c2 = 1;
      if (procs[i].ppid != parent_pid) ppid_ok = 0;
      if (strcmp(procs[i].state, "SLEEPING") == 0 || strcmp(procs[i].state, "RUNNABLE") == 0) {
        state_ok = 1;
      }
    }
  }
  ...
  if (!found_c1 || !found_c2 || !ppid_ok || !ticks_ok || !sched_ok || !state_ok) {
