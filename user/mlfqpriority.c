...
main(int argc, char *argv[])
{
  ...
  printf("  - Direct: struct proc_info exposes 'priority' (0=Q0, 1=Q1, 2=Q2)\n");
  printf("  - Indirect: 'cpu_ticks' and 'num_sched' confirm time slice progression\n");
  ...
  for (int step = 1; step <= 10; step++) {
    ...
    if (get_proc(cpid, &info)) {
      printf("  Step %d: prio=Q%d ticks=%ld sched=%d state=%s\n",
             step, info.priority, info.cpu_ticks, info.num_sched, info.state);
      if (info.priority == 0) seen_q0 = 1;
      if (info.priority == 1) seen_q1 = 1;
      if (info.priority == 2) seen_q2 = 1;
    } else {
  ...
  if (!seen_q1 || !seen_q2) {
    printf("mlfqpriority: FAIL - child did not demote through MLFQ queues as expected\n");
    exit(1);
  }

  printf("mlfqpriority: PASS - demotion timeline matches MLFQ queue specifications\n");
